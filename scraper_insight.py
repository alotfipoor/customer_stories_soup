...
base_url = "https://insightsoftware.com/customer-stories/"
...
def extract_and_print_data(driver, link):
    ...
    company_name_element = soup.find('span', text='Company').find_next_sibling('span')
    company_name = company_name_element.text.strip() if company_name_element else "Company name not found"
    data['company'] = company_name

    content_elements = soup.select('div.story-content.mb-4')
...
def get_story_links(driver):
    ...
    try:
        WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'div.card-body a'))
        )
        html = driver.page_source
        soup = BeautifulSoup(html, "html.parser")
        story_links_elements = soup.select('div.card-body a')
        story_links = [link['href'] for link in story_links_elements if link.has_attr('href')]
        return story_links
...
with open('customer_stories_insight.json', 'w') as outfile:
