...
def get_story_links(driver):
    ...
    try:
        WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'div.card.customer-card.card-elements a'))
        )
        html = driver.page_source
        soup = BeautifulSoup(html, "html.parser")
        story_links = soup.select('div.card.customer-card.card-elements a')
